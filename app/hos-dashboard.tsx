...
export function HOSDashboard({ ws, navigate, page, mode = 'hos' }: HOSProps) {
  ...
  return <div className="hos-dashboard">
    ...
    <div className="hos-tabs" role="tablist">{(['Home', 'Calendar', 'Inquiries'] as const).map((item) => <button key={item} className={tab === item ? 'active' : ''} onClick={() => setTab(item)}>{item}</button>)}</div>
    ...
    {tab === 'Home' && <>
      ...
      <div className="hos-schoolwide"><h2>School-wide dashboard</h2><p>All existing Admin analytics, records and school views remain available to you.</p><AdminMasterDashboard ws={ws} navigate={navigate} select={() => undefined} /></div>
    </>}
