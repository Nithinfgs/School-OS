...
export function ReportCards({ ws, role, navigate }: { ws: any; role: string; navigate?: (page: string) => void }) {
  ...
  const downloadReport = (r: Report) => {
    ...
    if (!printWindow) {
      // Fallback to text file if popups are blocked
      const body = `SchoolOS Official Academic Report\nStudent: ${r.studentName}\nClass: ${r.className}\nSubject: ${r.subject}\nTerm Grade: ${r.grade}\nEffort: ${r.effort}\n\nTeacher Evaluation:\n${r.comment}\n\nDate Issued: ${new Date().toLocaleDateString()}\nWestbridge International`;
      const link = document.createElement('a');
      link.href = URL.createObjectURL(new Blob([body], { type: 'text/plain' }));
      link.download = `${r.studentName.replace(/\s+/g, '_')}-${r.subject.replace(/\s+/g, '_')}-Report.txt`;
      link.click();
      URL.revokeObjectURL(link.href);
      return;
    }
    // Build a printable document with DOM APIs so school content is never interpreted as HTML.
    printWindow.opener = null;
    const doc: Document = printWindow.document;
    doc.title = `${r.studentName} - ${r.subject} Report Card`;
    const style = doc.createElement('style');
    style.textContent = 'body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;padding:40px;color:#1e293b;max-width:720px;margin:auto;line-height:1.5}.header{border-bottom:2px solid #0f172a;padding-bottom:20px;margin-bottom:28px}.school{font-size:24px;font-weight:800}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.card,.comment{border:1px solid #e2e8f0;border-radius:8px;padding:14px;margin:10px 0;background:#f8fafc}.card small{display:block;color:#64748b;text-transform:uppercase}.comment{background:#fff;white-space:pre-wrap}.actions{margin-bottom:18px}@media print{.actions{display:none}}';
    doc.head.appendChild(style);
    const body = doc.body;
    const add = (tag: string, text: string, className?: string) => { const node = doc.createElement(tag); node.textContent = text; if (className) node.className = className; body.appendChild(node); return node; };
    const actions = add('div', '', 'actions');
    const printButton = doc.createElement('button'); printButton.type = 'button'; printButton.textContent = 'Print / Save as PDF'; printButton.addEventListener('click', () => printWindow.print()); actions.appendChild(printButton);
    const closeButton = doc.createElement('button'); closeButton.type = 'button'; closeButton.textContent = 'Close'; closeButton.addEventListener('click', () => printWindow.close()); actions.appendChild(closeButton);
    const header = add('div', '', 'header');
    const school = doc.createElement('div'); school.className = 'school'; school.textContent = 'Westbridge International School'; header.appendChild(school);
    const subtitle = doc.createElement('div'); subtitle.textContent = `Official Academic Progress Report · ${activeCycle.name}`; header.appendChild(subtitle);
    const grid = add('div', '', 'grid');
    [['Student Name', r.studentName], ['Class / Cohort', r.className], ['Subject', r.subject], ['Issue Date', new Date().toLocaleDateString()], ['Achievement Grade', `${r.grade} / 7`], ['Effort Assessment', r.effort]].forEach(([label, value]) => { const card = doc.createElement('div'); card.className = 'card'; const small = doc.createElement('small'); small.textContent = label; const strong = doc.createElement('strong'); strong.textContent = value; card.appendChild(small); card.appendChild(strong); grid.appendChild(card); });
    add('h3', 'Teacher Evaluation & Learning Evidence');
    add('div', r.comment, 'comment');
    doc.close();
  };
  return <section className="report-cards-page">
    ...
    {tab === 'Templates' && <div className="panel report-panel"><h2>Report templates</h2><p className="muted">Configure the presentation of published reports without changing report data.</p><div className="template-card"><FileText size={24} /><div><strong>Westbridge International · Standard report</strong><small>Student details · subject table · attendance · signatures · grading legend</small></div><Button variant="outline" onClick={() => setMessage('Template editing is not available until templates are stored in the backend.')} disabled>Edit template</Button></div></div>}
