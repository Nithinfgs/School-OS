...
class SupabaseTransportRepository extends SupabaseEntityRepository {
  ...
  async save(entity:any) {
    ...
    if(['BUS_ARRIVAL','BUS_LATE'].includes(entity.recordType) && details.busId) {
      ...
      const {error}=await this.client.from('bus_departure_records').upsert({organization_id:entity.organizationId,bus_id:details.busId,departure_date:details.date,scheduled_departure_time:details.scheduledDepartureTime||null,actual_departure_time:details.actualTime||null,departure_status:details.status,delay_minutes:details.lateMinutes||null,delay_reason:details.lateReason||null,notes:details.notes||null,recorded_by:entity.createdBy,tracked_record_id:entity.id},{onConflict:'organization_id,bus_id,departure_date'}); if(error) throw error;
      await this.client.from('transport_activity').insert({organization_id:entity.organizationId,activity_type:entity.recordType,bus_id:details.busId,actor_id:entity.createdBy,details:{status:details.status,time:details.actualTime,delayMinutes:details.lateMinutes,reason:details.lateReason},tracked_record_id:entity.id});
